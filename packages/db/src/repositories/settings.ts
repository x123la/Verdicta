...
export class SettingsRepository {
  ...
  async upsert(input: UpdateSettingsInput): Promise<Settings> {
    ...
    const row = {
      ...
      defaultModelProvider: input.defaultModelProvider ?? existing?.defaultModelProvider ?? "local",
      defaultModelName: input.defaultModelName ?? existing?.defaultModelName ?? "auto",
